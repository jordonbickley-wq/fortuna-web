...
function createPendingManualEntitlement({ userId, drawDate, payerNote, amountSatang }) {
  const record = {
    id: `ent_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
    userId,
    drawDate,
    payerNote: (payerNote || '').slice(0, 200),
    amountSatang: amountSatang || null,
    chargeId: null,
...
// Amounts currently reserved by unpaid manual requests, so a new request
// is never given the same reference amount as an outstanding one.
function getPendingManualAmounts() {
  return (db.get('entitlements').filter({ status: 'pending_manual' }).value() || [])
    .map((e) => e.amountSatang)
    .filter(Boolean);
}
...
// ---- visitor profile (name + birth day of week) ----
// Deliberately minimal: just enough to personalise the experience, no
// more. No email, no phone, no birth year - a day of the week and a
// display name can't identify anyone on their own.
db.defaults({ profiles: [] }).write();

function saveProfile({ userId, displayName, birthDayIndex }) {
  const existing = db.get('profiles').find({ userId }).value();
  const clean = {
    displayName: String(displayName || '').trim().slice(0, 40),
    birthDayIndex: Number.isInteger(birthDayIndex) && birthDayIndex >= 0 && birthDayIndex <= 6 ? birthDayIndex : null,
    updatedAt: new Date().toISOString(),
  };

  if (existing) {
    db.get('profiles').find({ userId }).assign(clean).write();
    return db.get('profiles').find({ userId }).value();
  }
  const record = { userId, ...clean, createdAt: new Date().toISOString() };
  db.get('profiles').push(record).write();
  return record;
}

function getProfile(userId) {
  return db.get('profiles').find({ userId }).value() || null;
}
